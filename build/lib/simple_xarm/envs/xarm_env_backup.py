...
class XarmEnv(gym.Env):
    ...
    def calculateReward(self):
        ...
        distance_obj_goal_z = self.getDistance(current_obj[2], current_pos[2])-0.07  #offset
        reward_distance_gripper_obj = (1-(3*(distance_obj_goal_x+distance_obj_goal_y))- distance_obj_goal_z)
        
        distance_obj_goal = self.getDistance(goal_z, current_obj[2])
        reward_distance_obj_goal = (0.45-distance_obj_goal)*2

        left_lower_bound = current_obj-(0.03,0.06,0.03)
        left_upper_bound = current_obj+(0.03,0.0,0.05)
        right_lower_bound = current_obj-(0.03,0.0,0.03)
        right_upper_bound = current_obj+(0.03,0.06,0.05)
        if (left_lower_bound < left_finger).all() and (left_finger< left_upper_bound).all() and (right_lower_bound < (right_finger)).all() and (right_finger < right_upper_bound).all():
            ...
            if self.current_timeStep % 10 == 0:
                if dist_fing<0.08:
                    print("************grab**********")
            reward_gripper = (1.2-dist_fing*10)*2 #0=open
